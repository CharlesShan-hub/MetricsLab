...
def _ssim(
    ...
    return (num / (den + eps), sigma1_sq, sigma2_sq)
...
def q_w(A, B, F, window_size=11, eps=1e-10):
    ...
    def sigma2(A):
        X = kornia.filters.filter2d(A,torch.tensor([[[1,0,-1],[1,0,-1],[ 1, 0,-1]]]),border_type='constant')
        Y = kornia.filters.filter2d(A,torch.tensor([[[1,1, 1],[0,0, 0],[-1,-1,-1]]]),border_type='constant')
        A = torch.sqrt(X**2 + Y**2 + eps)
    ...
    print(torch.mean(sigma2A_sq))
    print(torch.mean(sigma2B_sq))

    # buffer=sigma2A_sq+sigma2B_sq;
